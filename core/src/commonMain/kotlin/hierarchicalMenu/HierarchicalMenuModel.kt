...
open class HierarchicalMenuModel<T> {
...
fun <T> HierarchicalMenuModel<T>.connectSearcher(searcher: Searcher) {
    selectionListeners += { searcher.search() }
}

fun <T> HierarchicalMenuModel<T>.connectView(view: HierarchicalView<T>) {
    hierarchyListeners += (view::setHierarchy)
    selectionListeners += (view::setSelected)
    view.onClickRefinement(::onSelectedRefinement.get())
}

fun <T> HierarchicalMenuModel<T>.connectViews(views: List<HierarchicalView<T>>) {
